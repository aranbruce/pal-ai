...
export default function WeatherDisplay({ data }: WeatherDisplayProps) {
  ...
  return (
    <div className="text-weather-foreground mb-4 flex max-w-full flex-col items-start gap-4 rounded-lg bg-[image:var(--weather-gradient)] p-3 md:p-4">
        ...
        {data.hourly.slice(0, 7).map((hour, index) => (
          <div className="flex w-8 flex-col items-center gap-1" key={index}>
            <h5 className="text-xs opacity-90">
              {index === 0
                ? "Now"
                : ((data.currentHour + index) % 24).toString().padStart(2, "0")}
            </h5>
            <WeatherImage
              height={32}
              width={32}
